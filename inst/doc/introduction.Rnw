...
\section{The Logbook}
\label{sec:logbook}
\warnbox{Deprecated}{The logbook is now DEPRECATED and the functionality will be removed in the future.\\
  This feature has never seen much use, but slows down \phy considerably.}  Slot \Rcode{@log} of \chy
objects is intended to keep track of the history of the object.  This logbook part of the output of
the \Rfunction{summary}, and can also be retrieved by \Rfunction{logbook}.
<<logbook>>=
logbook (flu)
@
...
@ 
\includegraphics[width=.5\linewidth]{introduction-fig--ggplot-do.pdf}
...
@
\begin{figure}
\subfloat[\label{fig:clust:dend}]{\includegraphics[width=0.33\textwidth]{introduction-fig--dend}}
\subfloat[\label{fig:clust:map}]{\includegraphics[width=0.33\textwidth]{introduction-fig--clustmap}}
\subfloat[\label{fig:clust:spc}]{\includegraphics[width=0.33\textwidth]{introduction-fig--clustmean}}
...
\index{options!log} A first possibility is switching of the automatic logging of how the objects are
transformed. This is now the default setting of the option as the logbook will 

Logging involves appending rows to the \df in slot \Rcode{@log}.  While the absolute
amount of time needed to add a logbook entry is small, it may be executed very often (e.g. during
each call of \Rfunction{[}). 
<<speed2>>=
hy.setOptions (log = FALSE)
tmp <- chondro [1 : 50]
system.time ({
